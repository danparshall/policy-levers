...
Breaking our notion of machine intelligence into vocabulary that align with possible interventions makes it easier to see which aspects have technical solutions, which have policy solutions, and probably points us to places where there are no direct solutions at all. A spec sheet works for a submarine partly because it's built to mission requirements, and partly because a submarine doesn't have its own goals. But well-chosen vocabulary can help avoid our abstractions springing a leak, right when we need them most.
