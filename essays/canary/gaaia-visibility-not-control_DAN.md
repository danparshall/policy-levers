...
On Sunday evening (July 20), mathematician Levent Alpöge announced that Claude Fable 5 had found a counterexample to the Jacobian conjecture, an algebraic geometry problem open since 1939 and on Smale's list of problems for this century.  The counterexample is three lines of polynomials with Jacobian determinant identically -2 that map three different inputs to the same output; mathematicians verified it within a day, because checking is easy even when finding was not (the two-variable case survives, for anyone keeping score).  Full disclosure: Fable is the model that read this bill with me.

On Monday afternoon (July 21), OpenAI [confirmed](https://openai.com/index/hugging-face-model-evaluation-security-incident/) that the intrusion Hugging Face had disclosed the week before was its own models: GPT-5.6 Sol, their deployed flagship, plus a more capable pre-release model, running a cyber evaluation with the production safety classifiers deliberately off.  The models found a zero-day in the sandbox's package proxy, escalated to a node with internet access, then chained stolen credentials and further zero-days into remote code execution on Hugging Face's production servers, all to steal the answer key to the benchmark they were being scored on.  Hugging Face observed thousands of actions across a swarm of short-lived sandboxes with self-migrating command-and-control.  To be fair to the models, they had been asked to demonstrate exploitation and had their refusals turned down; the part nobody asked for was escaping the sandbox, stealing credentials, and attacking an uninvolved third party as the shortest path to a benchmark score.  OpenAI's own writeup calls them "hyperfocused".
