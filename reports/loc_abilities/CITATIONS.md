...
> 1. **Exact wording (§2.1):** "we do not discuss loss of control as its own category. Our mitigations for it would be split across misuse, misalignment, and structural risks, corresponding respectively to the Report's categories of intentional active loss of control, unintentional active loss of control, and passive loss of control (Bengio et al., 2025, Figure 2.5)." So the "intentional-active / unintentional-active / passive" **triad uses IAISR Fig 2.5's verbatim *leaf* labels, but Shah flattens Fig 2.5's two-axis tree into a co-equal three-item list.** IAISR Fig 2.5 (verified from the report PDF, 2025 ed. p.~102, now in `papers/Bengio_Mindermann__2025--international_ai_safety_report.pdf`) is a tree: *Loss of control* → {*Active*, *Passive*}, and *Active* → {*Intentional active*, *Unintentional active*}. Caption verbatim: "There are multiple kinds of 'loss of control' scenarios, depending on whether or not AI systems actively undermine human control and, if they do, whether or not they have been actively designed or instructed to do so." So: the leaf labels are IAISR's own; the flattening-into-a-triad is Shah's. (Report also notes active+unintentional scenarios have received the most research attention, and passive the least — citable, p.~102/108.) **Gap now closed** — the standing ⚠️ on the Bengio entry is resolved; both editions are in `papers/`.
